...
**Audit update 2026-07-10:** this roadmap remains the design history, but open release blockers and their current evidence are canonical in [agentic-reliability-issues-2026-07.md](agentic-reliability-issues-2026-07.md). Fix MAULER-AR-001/002 before extending the loop, isolate Agent Eval under MAULER-AR-003 before promoting it as a gate, and use MAULER-AR-004/005 for completion and benchmark parity work.
...
| Item | Status | Where |
|---|---|---|
| Generic duplicate read-only tool detector | [done] | `internal/app/agent_loop_stability.go` `repeatedIdenticalReadBlock`, registered in `preToolRecoveryRules`; escalation in `repeatedPreToolRecoveryIgnored` |
| `tool_choice=required` on inspection/ops first turn | [done] | `toolChoiceFor` + empty-defs guard in `toolDefsAndChoiceForTurn` |
| Model tool-calling-tier guardrail (doctor) | [done] | `doctor.go` `addModelTierCheck` + `modelParamBillions` |
| llama.cpp agent launch-flag advisory (doctor) | [done] | `doctor.go` `addLlamacppAgentFlagAdvisory` |
| Auto-lint after edits (`go vet`/`py_compile`/`bash -n`) | [done] (pre-existing) | `mutation_verifier.go` `appendLint`->`lintFile` |
| SSE quirk handling, inline-tool repair, force-no-think, truncation->chunked-write, compaction + memory re-injection, redaction, rollback | [done] (pre-existing) | `internal/llm/stream.go`, `internal/app/app.go` |

---

# Tier 1 - do these first (make every future change safe + measurable)

## R1. Agentic-loop eval harness [done]
...
## R2. `settings.Validate()` - clamp on load, not just warn [done]
...
## R4. Global wall-clock task budget [done]
...
## R5. Sub-agent for read-heavy sub-tasks [done]
...
## R6. Frontier escalation fallback (+ finish the Anthropic backend) [done]
...
## R7. Grammar-constrained tool arguments (llama.cpp) [test]
...
## R8. Plan-then-execute via the todo tool [done]
...
## R9. Periodic goal re-injection [done]
...
## R10. Loop-health telemetry [done]
...
All [done] items remain green, R1 eval suite passes on the active profile, R2 guarantees no out-of-range config reaches the loop, and R3/R4 mean no run can run away or be lost. That is the bar.
