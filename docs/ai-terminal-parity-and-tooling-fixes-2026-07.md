...
### P0-1. Render a headless VT screen for the agent instead of a flat line log ✅ first pass
...
- Add a VT emulator dependency. `github.com/hinshun/vt10x` or `github.com/charmbracelet/x/vt` are the practical choices (a `go-ansi-parser` indirect dep already exists but is not a screen model). Size the screen to the PTY size (`OpenShell` starts at 200×50, `app.go:6089`; keep it synced with `ShellResize`).
...
### P0-2. Exact command-completion detection via shell integration markers (kill the poll-loop bug class) ✅ first pass
...
### P0-3. Detect in-place screen changes, not just new lines ✅ first pass
...
### P0-4. Full keystroke vocabulary (drive TUIs/REPLs like a human) ✅ first pass
...
### P1-1. Collapse the overlapping shell entry points 🔨 partial
...
### P1-2. Prompt-glyph detection is bash-only across the codebase ✅ first pass
...
### P1-3. `terminal_read` conflates "screen" and "history" ✅ first pass
...
### P1-4. Confirm `run_script` executes over the tool registry, sandboxed ✅ first pass
...
|---|------|----------------|--------|
| P0-1 | Headless VT screen for the agent | The core 1:1-with-human gap; unlocks all TUIs/REPLs | ✅ first pass |
| P0-2 | OSC-133 completion + real exit code | Kills the poll-loop-on-finished-command bug on every backend | ✅ first pass |
| P0-3 | Detect in-place screen change | Busy commands stop looking idle to the agent | ✅ first pass |
| P0-4 | Full keystroke vocabulary | Actually navigate less/vim/msfconsole/history | ✅ first pass |
| P1-1 | Collapse overlapping shell tools | Fewer wrong tool choices (esp. small models) | 🔨 partial |
| P1-2 | De-bash-ify prompt detection | PowerShell/zsh parity | ✅ first pass |
| P1-3 | Split screen vs history in read | Clean TUI reads | ✅ first pass |
| P1-4 | Verify run_script sandbox/registry | Programmatic calling must be real+safe | ✅ first pass |
| P2-* | Blocking mode, structured results, cwd, patch tool, scrollback search | Match Claude/Codex ergonomics | ✅ first pass for wait_for/cwd/contracts/search; patch tool open |
