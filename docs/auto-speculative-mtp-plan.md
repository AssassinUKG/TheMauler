...
- **2026-06-23 (a)** — Verified the bridge contract. Found and fixed the real blocker: the
  ...
  correct — the earlier "draft-mtp vs mtp" worry is RESOLVED, no Mauler change needed). See §0.1.
- **2026-06-23 (b)** — Implemented Phase 1 (auto-detect + auto-apply + badge). See §3.5 for the
...
- **2026-06-23 (c)** — Implemented Phase 3 (stability guard) + the carried-over bridge-build Doctor
  check. See §4.1. TheMauler rebuilt (bridge unchanged this round). Acceptance-rate floor deferred
  to Phase 3b (needs a bridge metric) — the truncation-signature auto-fallback is in.
- **2026-06-23 (d)** — Implemented Phase 2 (auto-tune n + calibration cache). See §5.1. TheMauler
...
- **Dense vs MoE:** dense models gain 1.4–2.2× (Qwen3.6-27B). MoE gains only ~1.15–1.25×
...
runtime watchdog (acceptance rate, </think> signature) ──► auto-fallback (Phase 3)
        │
        ▼
UI badge: "MTP · auto · n=3 · 1.8×"  (Phase 4)
```
...
- `src-tauri/src/engine/process.rs` — ungated `--spec-type`/`--spec-draft-n-max` from the draft-model
  block (self-MTP fix, §0.1); added a focused `target: "speculative"` launch log; added regression
...
- **Bridge-build Doctor check (§0.1 item 4) NOT yet added** — if the user runs an *old* bridge
...
Reuse `BenchmarkProfileWithCases` to sweep `SpecDraftNMax ∈ {1,2,3,4,5}` on a tiny fixed prompt
...
Expected: 27B dense lands on n≈3; 35B-A3B MoE smaller gain, possibly n=2. If `Speedup < ~1.05×`,
...
- `internal/app/spec_calibration.go` (+ `spec_calibration_test.go`):
  ...
  - `CalibrateSpec(profileName)` — guards on `agentRunning`, requires llama.cpp + MTP-capable, then
    measures a spec-off baseline and sweeps `spec_draft_n_max ∈ {1..5}`, picks the fastest, caches
...
- `internal/app/autospec.go` — `autoApplySpec` now overrides `plan.NMax` with the cached `BestN`
  when enabled, and appends `· tuned n=X (Y× vs off)` to the reason. So calibrate once → every
...
- **MTP chip** in the Agent panel Status block (`frontend/src/components/AgentPanel.tsx`, the
  ...
  (`frontend/src/components/LiveOpsPage.tsx`). Format: `MTP · auto · n=3 · 1.8×`.
...
```
spec_type        = mtp        (internal "draft-mtp" — VERIFY bridge maps it; see §0)
...
1. ~~Verify bridge contract / self-MTP path~~ — **DONE 2026-06-23** (§0.1). Bridge `process.rs`
   fixed + tested; needs its own rebuild/commit in the InferenceBridge repo.
2. ~~Phase 1 (auto-detect + auto-apply + badge)~~ — **DONE 2026-06-23** (§3.5). Both apps rebuilt.
   Outstanding sub-item: the bridge-build Doctor check (§3.6 / §0.1 item 4).
3. ~~Phase 3 (stability guard) + bridge-build Doctor check~~ — **DONE 2026-06-23** (§4.1). The
   `</think>` truncation auto-fallback is in; the acceptance-rate floor is deferred to Phase 3b
   (needs a bridge metric).
4. ~~Phase 2 (auto-tune n + calibration cache)~~ — **DONE 2026-06-23** (§5.1). Cached best-n feeds
