...
## Agentic-flow
- [x] **3 + 4. Steer to terminal_run + persistent foothold.** Interactive tools used 0×; model re-ran the full exploit chain per command. Strengthen the system-prompt/preference guidance so live/long/exploit commands go through `terminal_run`, and add a hint to establish a persistent session instead of re-exploiting per command. `internal/app/app.go` (PreferTerminalTools block ~7559) + listener-style routing.
