...
**Author:** run-ledger post-mortem. **Status:** 🔨 spec ready, not started.
...
2. **~50% of model calls produce no tool call**, every run (`model ≈ 2× tool`; protocol-recovery ≥
...
## F1. Normalize the repeat/dedup key so pager-only variations collide 🔨 ⭐ do first
...
## F2. Deterministic 3xx-redirect hint on shell/http_probe results 🔨
...
## F3. Loop circuit-breaker on collapsed stability 🔨
...
**Acceptance.** A run that repeats the same command with the same result 2–3× gets a corrective
injection, and if it persists, auto-pauses with reason `loop_circuit_breaker`.

---

## F4. Pin one authoritative target; warn on off-target probes 🔨
...
## F5. Cut orientation overhead (inject recap/progress; skip empty globs) 🔨
