...
func TestStorageItemsIncludeClearableLogsAndBenchmarks(t *testing.T) {
	...
	for _, id := range []string{"config", "state_db", "benchmark_runs", "ledger", "session_recall", "checkpoints"} {
		if byID[id].ID == "" {
			t.Fatalf("missing storage item %q in %#v", id, items)
		}
	}
	if !byID["benchmark_runs"].Clearable || !byID["ledger"].Clearable {
		t.Fatalf("expected benchmark and ledger to be clearable: %#v", byID)
	}
